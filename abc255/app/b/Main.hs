...
solve (n,_,as,xys) =
    let
        lights = map (\a -> xys !! (a - 1)) as
        unLights = map (\i -> xys !! (i - 1)) . filter (not .(`elem` as)) $ [1..n]
        -- 距離の２乗
        distSq :: [Int] -> [Int] -> Int
        distSq [x1,y1] [x2,y2] = (x1-x2)^(2::Int) + (y1-y2)^(2::Int)
    in if null unLights
          then 0
          else sqrt $ fromIntegral $ maximum [minimum [distSq l u | l <- lights] | u <- unLights]


{-# INLINE decode #-}
decode :: [[I]] -> Dom
decode = \ case
    [n, k]:as:xys -> (n, k, as, xys)
