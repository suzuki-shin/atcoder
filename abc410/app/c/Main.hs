...
長さ N の整数列 A があり、最初 Ai​=i です。以下のクエリを全部で Q 個処理してください。

    タイプ 1 : Ap​ を x に変更する。
    タイプ 2 : Ap​ を出力する。
    タイプ 3 : 「 A の先頭の要素を末尾にする」という操作を k 回繰り返す。
        厳密には、 A を (A2​,A3​,…,AN​,A1​) に置き換えることを k 回繰り返す。
...
-}
{-
@tags: [仮想ローテーション オフセット管理 STモナド MVector foldM]
@note: 実際に配列を回転させず、論理オフセット ofs を foldM のアキュムレータで持ち回す。
       phys ofs p = (p - 1 + ofs) `mod` n で論理1-indexed p を物理0-indexed に変換。
       タイプ3 は ofs に k を足して mod n するだけで O(1)。タイプ1/2 は VUM.write / VUM.read。
       step は runST の do ブロック内の let に置くと av/n/phys がクロージャで見え、
       foldM step (0, []) xs で状態 (オフセット, 出力リスト) を畳み込める。
       注意: case alternatives は step より深くインデントしないとパースエラー。
-}
solve :: Solver
solve (n,_,xs) =
  res
  where
    res = runST $ do
      av <- VUM.generate n (+1)
      let phys ofs p = (p - 1 + ofs) `mod` n -- 論理1-indexed p -> 物理0-indexed
      let step av (!ofs, !out) q = case q of
            [1,p,x] -> do
              VUM.write av (phys ofs p) x
              pure (ofs, out)
            [2,p] -> do
              v <- VUM.read av $ phys ofs p
              pure (ofs, v:out)
            [3,k] -> do
              pure (ofs + k, out)
            _ -> error "xxx"
      (_finalOfs, outRev) <- foldM (step av) (0 :: Int, [] :: [Int]) xs
      pure (reverse outRev)
