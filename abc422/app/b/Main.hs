...
{-# INLINE solve #-}
solve :: Solver
solve (n,m,grid) = yn res
  where
    ary :: Array (Int, Int) Char
    ary = listArray @Array ((1,1),(n,m)) $ concat grid
    neighborIndexes :: (Int, Int) -> [(Int, Int)]
    neighborIndexes (r,c) = filter (inRange ((1,1),(n,m))) [(r+1,c),(r-1,c),(r,c+1),(r,c-1)]
    -- 座標iの隣のマスのうち#のものが２つもしくは4つであるかどうか
    ok :: (Int, Int) -> Bool
    ok idx = len == 2 || len == 4
      where
        neighbors = map (ary !) $ neighborIndexes idx
        len = length $ filter (== '#') neighbors
    res = all (ok . fst) (filter (\(_,v) -> v == '#') $ assocs ary)
