...
```
atcoder/
  abc{NNN}/                          # コンテストごとのディレクトリ
    package.yaml                     # Stack プロジェクト定義
    building                         # ビルドスクリプト (stack build)
    checking                         # テストスクリプト (oj t)
    executing                        # 実行スクリプト (stack exec)
    setting <problem>                # 問題切り替え + HLS設定
    ghcing                           # GHCi 起動
    initializing                     # acc new でコンテスト初期化
    resetting <problem>              # .curname を切り替え
    app/
      Main.hs                        # テンプレート (新規問題のコピー元)
      a/Main.hs                      # 問題A の解答
      a/tests/sample-{N}.in          # サンプルケース
      b/Main.hs                      # 問題B の解答
      ...
      zzz/add-entry/Main.hs          # package.yaml にエントリ追加するツール
```
...
1. `./setting b` → 問題 b に切り替え
2. `app/b/Main.hs` を編集
3. `./building` → ビルド
4. `./checking` → サンプルケースでテスト (`oj t`)
5. AC したら `@tags` と `@note` をコメントに記録
...
`solve` 関数（およびそのヘルパー関数）を読み、「知識ベース: AtCoder 頻出パターン集」に該当するパターンがないかチェックする。該当するパターンがあれば、以下を指摘する:
...
- L42-45 の手書き二分探索 → Bonsai の `bisect` に置き換え可能
- L30 の `Data.List.sort` → Bonsai の `sort'` (Vector ベースで高速) に置き換え可能
...
AtCoder振り返りNotionガイドに基づいた振り返りテンプレートを生成する:
...
4. ビルド・テスト実行を提案する場合は以下のコマンドを使う:
   ```bash
   ./building   # ビルド
   ./checking   # サンプルケースでテスト
   ./executing  # 標準入力から実行
   ```
...
このファイル (`ac-haskell.agent.md`) を編集した際は、**毎回以下のチェックを実施**すること:
