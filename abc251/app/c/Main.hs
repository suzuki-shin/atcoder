...
POJ に N 回の提出がありました。早い方から i 番目の提出では文字列 Si​ が提出されて、得点は Ti​ でした。(同じ文字列が複数回提出される場合もあります)
...
solve (_, sts) =
  maxIx
  where
    (maxIx, _, _) = foldl' f (0, S.empty, -1) $ zip [1..] sts
    f (maxIx, ss, maxP) (ix, (s,t))
      | S.member s ss = (maxIx, ss, maxP)
      | t > maxP      = (ix, ss', t)
      | otherwise     = (maxIx, ss', maxP)
      where
        ss' = S.insert s ss
