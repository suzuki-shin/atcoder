...
{-
問題文
英小文字からなる 3 つの文字列 S1 ,S2 ,S3 と、1、2、3 のみからなる文字列T が与えられます。
T の各文字に対応する文字列を連結してできる文字列を出力してください。より厳密には、以下の指示にしたがって文字列を出力してください。

1≤i≤∣T∣ を満たす整数 i に対し、文字列 si を次のように定める。
T の i 文字目が 1 のとき、S1
T の i 文字目が 2 のとき、S2
​T の i 文字目が 3 のとき、S3
​
...
{-# INLINE solve #-}
solve :: Solver
solve (ss, t) = concatMap ((ss!!) . pred . digitToInt) t
