...
solve (n, hs) = dp VG.! n
  where
    ...
    dp = VU.constructN (n+1) $ \v ->
      ...
      -- dp[1] = 0
      -- dp[2] = dp[1] + hv[2] - hv[1]
      -- dp[3] = min(dp[2] + hv[3] - hv[2], dp[1] + hv[3] - hv[1])
      -- dp[n] = min(dp[n-1] + hv[n] - hv[n-1], dp[n-2] + hv[n] - hv[n-2])
      in if | i <= 1 -> 0
            | i == 2 -> v VG.! 1 + abs((hv VG.! 2) - (hv VG.! 1))
            | otherwise ->
                min ((v VG.! (i-1)) + abs((hv VG.! i) - (hv VG.! (i-1)))) ((v VG.! (i-2)) + abs((hv VG.! i) - (hv VG.! (i-2))))
