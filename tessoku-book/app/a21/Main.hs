...
{-# INLINE solve #-}
solve :: Solver
solve (n, pas) = maximum [dp ! (i,i) | i <- [1..n]]
  where
    paAry :: Array Int (Int, Int)
    paAry = listArray (1,n) pas
    unreach = minBound `div` 2

    -- dp ! (l,r) は「ブロック l から r が残っている状態」の最大得点
    dp :: Array (Int, Int) Int
    dp = array ((1,1), (n,n)) [((l,r), compute l r)| l <- [1..n], r <- [1..n], l <= r]
...
decode :: [[I]] -> Dom
decode = \case
  [n] : as -> (n, map (\[p,a] -> (p,a)) as)
  _ -> invalid $ "toDom: " ++ show @Int 105
...
decode = \case
  nm : grid ->
    let [n, m] = map (read @Int) $ words nm
     in (n, m, grid)
  _x -> trace (show _x) invalid $ "toDom: " ++ show @Int 130
