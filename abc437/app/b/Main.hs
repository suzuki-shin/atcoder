...
{-# INLINE solve #-}
solve :: Solver
solve (h, w, n, ass, bs) = maximum $ map (\as' -> IS.size (as' `IS.intersection` bs')) ass'
  where
    ass' :: [IS.IntSet]
    ass' = map IS.fromList ass
    bs' :: IS.IntSet
    bs' = IS.fromList bs
...
decode :: [[I]] -> Dom
decode = \case
  [h,w,n] : rest -> (h,w,n,ass,bs)
    where
      ass = take h rest
      bs = concat $ drop h rest
  _ -> invalid $ "toDom: " ++ show @Int 86
