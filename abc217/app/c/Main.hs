...
{-
  @tags: [逆順列 順列 Array]
  @note: 順列 P の逆順列 Q を求める。p_i=v のとき q_v=i という対応をそのまま array に書けばよい: `array (1,n) [(p,i) | (i,p) <- zip [1..] ps]` で O(N) 1行。IntMap 版 (foldl' + IM.insert + IM.elems) も N≤2×10^5 なら十分高速 (O(N log N))。
-}
{-# INLINE solve #-}
solve :: Solver
-- array版
solve (n,ps) = elems inv
  where
    inv :: UArray Int Int
    inv = array (1,n) [(p,i) | (i,p) <- zip [1..] ps]

-- IntMapでfoldl'版
solve1 (n,ps) = IM.elems res
  where
    res = foldl' f IM.empty $ zip [1..] ps
    f :: IM.IntMap Int -> (Int,Int) -> IM.IntMap Int
    f acc (i,p) = IM.insert p i acc

-- sortで簡潔に
solve2 (n,ps) = map snd $ sort $ zip ps [1..]
