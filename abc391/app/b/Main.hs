...
@tags: [全探索 グリッド]
@note: N×Nグリッドから
M×Mパターンを探す。take/dropでサブグリッドを切り出してリスト比較。N≤50なのでO(N²M²)の全探索で十分
-}
{-# INLINE solve #-}
solve :: Solver
solve (n, m, ss, ts) =
  head [[a,b]| a <- [1..n-m+1], b <- [1..n-m+1], subGrid (a,b) == ts]
  where
    subGrid :: (Int, Int) -> [[Char]]
    subGrid (a, b) = map (take m . drop (b - 1)) . (take m . drop (a - 1)) $ ss
