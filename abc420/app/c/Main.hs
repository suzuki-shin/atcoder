...
type Dom   = (Int, Int, VU.Vector Int, VU.Vector Int, [(Char, Int, Int)])
type Codom = [Int]
...
decode :: [[I]] -> Dom
decode = \case
  nq : as : bs : qs -> (n, q, readInts as, readInts bs, map parseQ qs)
    where
      [n,q] = map read $ words nq
      readInts = VU.fromList . map read . words
      parseQ s = let (c:' ':rest) = s
                     [x', v'] = words rest
                 in (c, read x', read v')
...
solve (n, q, as0, bs0, qs) = runST $ do
  ...
  reverse . snd <$> foldM (\(!s, acc) (c, x, v) -> do
    ...
    let oldMin = min a b
    case c of
      'A' -> VUM.write av i v >> let s' = s - oldMin + min v b in return (s', s' : acc)
      _   -> VUM.write bv i v >> let s' = s - oldMin + min a v in return (s', s' : acc)
    ) (initSum, []) qs
    -- sumK = sum [min (av VG.! (k-1)) (bv VG.! (k-1))|k <- [1..n]]
