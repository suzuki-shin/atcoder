...
{-# INLINE solve #-}
solve :: Solver
solve (n,m,ss) = IM.keys ans
  where
    pointOf = IM.fromList $ zip [1..] $ map sum $ transpose $ map g $ transpose ss
    maxValue = maximum pointOf
    ans = IM.filter (== maxValue) pointOf
    -- 多い方を1にする
    -- >>> g "00111"
    -- [0,0,1,1,1]
    -- >>> g "10010"
    -- [0,1,1,0,1]
    g :: String -> [Int]
    g s = let s' = map digitToInt s
              win = if sum s' > length s `div` 2 then 0 else 1
          in map (\x -> if win == x then 1 else 0) s'
