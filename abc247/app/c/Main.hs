...
列 Sn​ を次のように定義します。
    ...
    S1​ は 1 つの 1 からなる長さ 1 の列である。
    Sn​ (n は 2 以上の整数) は Sn−1​, n, Sn−1​ をこの順につなげた列である。

たとえば S2​,S3​ は次のような列です。

    S2​ は S1​, 2, S1​ をこの順につなげた列なので 1,2,1 である。
    S3​ は S2​, 3, S2​ をこの順につなげた列なので 1,2,1,3,1,2,1 である。

N が与えられるので、列 SN​ をすべて出力してください。
制約
