...
-- encode r = [[r]]
-- encode = map (:[])
encode = map (\(i, js) -> show i <> ": " <> "{" <> (intercalate ", " (map show js) <> "}") )

{-
問題文
頂点数 N、辺数 M のグラフが与えられます。頂点には 1 から N までの番号が付けられており、
i 番目の辺は頂点 Ai と Bi を双方向に結んでいます。 それぞれの k について、「頂点 k と隣接している頂点の番号」をすべて出力するプログラムを作成してください。
...
solve :: Solver
solve (n, _, xs) =
  let edges = concatMap toEdges xs
      adj :: Array Int [Int]
      adj = accumArray (flip (:)) [] (1, n) edges
   in [(i, sort (adj ! i)) | i <- [1 .. n]]
  where
    toEdges = \case
      [a, b] -> [(a, b), (b, a)]
      _ -> invalid $ "edge: " ++ show @Int __LINE__
