...
import Data.Massiv.Array (HasCallStack)
...
-- | いもす法（Array版）
-- >>> imosV 8 [(2,3),(3,6),(5,7),(3,7),(1,5)] -- [(li, ri)] 参加者iがli日目からri日目まで参加する
-- [0,1,2,4,3,4,3,2,0,0]
imosV :: Int -> [(Int,Int)] -> [Int]
imosV d lrs = VU.toList cumSum
  where
    -- 1. 差分更新用のリストを作成
    -- (L, +1), (R+1, -1)
    updates :: [(Int, Int)]
    updates = concatMap (\(l, r) -> [(l, 1), (r + 1, -1)]) lrs
    -- 2. 初期配列(0)に対して差分を適用 (accum)
    -- サイズは d+2 (1-based index で d+1 までアクセスするため余裕を持つ)
    diff :: VU.Vector Int
    diff = VU.accum (+) (VU.replicate (d + 2) 0) updates
    -- 3. 累積和をとる (scanl1)
    -- 必要な範囲 (1..d) をスライスしてリスト化
    cumSum :: VU.Vector Int
    cumSum = VU.scanl1 (+) diff
