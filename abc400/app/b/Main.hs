...
solve :: Solver
solve (n,m) =
  if n ^ m > 10^9 then "inf" else maybe "inf" show mRes
  where
    mRes = foldl' f (Just 0) [0..m]
    f :: Maybe Integer -> Integer -> Maybe Integer
    f (Just acc) i = if acc' > 10^9 then Nothing else Just acc'
      where
        acc' = acc + n ^ i
    f Nothing _ = Nothing
