...
{-
@tags: [シミュレーション IntSet]
@note: N×Mのグリッドを実際に持つとMLE/TLE。行フラグ(IntSet)と個別セル(IntSet)を分離管理し、クエリ3で OR 判定。foldl'で純粋に書ける
-}
{-# INLINE solve #-}
solve :: Solver
solve (n,m,_,qs) = map yn $ reverse ans
  where
    (ans, _, _) = foldl' f ([], IS.empty, IS.empty) qs
    -- f :: ([Bool], IS.IntSet, IS.IntSet) -> [Int] -> ([Bool], IS.IntSet, IS.IntSet)
    f (ans, nms, ns) [1, x, y] = (ans, IS.insert (toIdx m (x, y)) nms, ns)
    f (ans, nms, ns) [2, x] = (ans, nms, IS.insert x ns)
    f (ans, nms, ns) [3, x, y] = ((IS.member (toIdx m (x, y)) nms || IS.member x ns) : ans, nms, ns)
    f acc _ = acc
