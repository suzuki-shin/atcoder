...
decode = \case
  n : rest -> (read n, cls)
    where
      cls = map ((\[c',l'] -> (head c', read l')) . words) rest
...
{-# INLINE solve #-}
solve :: Solver
solve (n, cls) = if sLen > 100 then "Too Long" else s
  where
    s = concatMap (\(c,l) -> replicate l c) cls
    sLen :: Integer
    sLen = sum $ map (toInteger . snd) cls
