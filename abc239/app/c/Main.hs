...
import Text.Printf
import qualified AtCoder.Extra.Semigroup.Permutation as S
...
xy 座標平面上の 2 つの格子点 (x1​,y1​),(x2​,y2​) からの距離がともに 5
​ である格子点は存在しますか？
注記

xy 座標平面上にある点のうち、x 座標と y 座標がともに整数である点を格子点と呼びます。
また、xy 平面上の 2 点 (a,b),(c,d) の距離をユークリッド距離 (a−c)2+(b−d)2
​ として定義します。

参考として、xy 平面の (0,0) の上に黒丸を、(0,0) からの距離が 5
​ となる格子点の上に白丸を書いた図は以下のようになります。(x,y いずれかが整数となる部分に目盛り線を引いています。)
...
solve (x1,y1,x2,y2) =
  yn $ not $ null res
  where
    root5s (x, y) = S.fromList [(x+dx,y+dy)| dx <- [-2,-1,1,2], dy <- [-2,-1,1,2], dx*dx+dy*dy == 5]
    res = S.intersection (root5s (x1, y1)) (root5s (x2, y2))
