...
整数の 2 つ組 (i,j) (1≤i<j≤N) のうち、j−i=Ai​+Aj​ を満たすものがいくつあるか求めてください。
制約
...
-}
{-
  @tags: [式変形 IntMap カウント]
  @note: j-i=A[i]+A[j] を j-A[j]=i+A[i] に変形し、左から走査しながらIntMapで i+A[i]
  の出現数を管理して O(N) で数える。未AC。
  -}
{-# INLINE solve #-}
solve :: Solver
solve (n,as) =
  ...
  where
    av = VU.fromList as
    res = foldl' f (IM.empty, 0) [1 .. n]
    f :: (IM.IntMap Int, Int) -> Int -> (IM.IntMap Int, Int)
    f (im, ans) k =
      let keyJ = k - av VG.! (k-1)
          keyI = k + av VG.! (k-1)
          cnt = IM.findWithDefault 0 keyJ im
      in (IM.insertWith (+) keyI 1 im, ans + cnt)
