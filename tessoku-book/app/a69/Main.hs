...
import Text.Printf
import qualified AtCoder.MaxFlow as MF
...
{-# INLINE solve #-}
solve :: Solver
solve (n,grid) = runST $ do
  -- 頂点数: ソース(0) + 生徒(1..N) + 席(N+1..2N) + シンク(2N+1)
  let s = 0 -- ソース
      t = 2 * n + 1 -- シンク
  mf <- MF.new (2*n+2)
  -- ソースから各生徒（容量１）
  forM_ [1..n] $ \i ->
    MF.addEdge_ mf s i 1

  -- 各席からシンク（容量1）
  forM_ [1..n] $ \j ->
    MF.addEdge_ mf (n+j) t 1

  -- 生徒iから席j（{C{i,j}} = '#' のとき、容量1）
  forM_ [0..n-1] $ \i ->
    forM_ [0..n-1] $ \j ->
      when (grid !! i !! j == '#') $
        MF.addEdge_ mf (i+1) (n+j+1) 1

  MF.flow mf s t maxBound
