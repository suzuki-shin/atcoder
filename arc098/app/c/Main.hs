...
solve :: Solver
solve (n, as) =
    let -- リーダーより左にいるWの数（向きを変える必要がある数）のベクター
        leftWNum = VU.fromList $ scanl' (\acc c -> if c == 'W' then acc + 1 else acc) 0 as
        -- リーダーより右にいるEの数（向きを変える必要がある数）のベクター
        rightENum = VU.fromList $ scanr (\c acc -> if c == 'E' then acc + 1 else acc) 0 as
    in minimum [(leftWNum VU.! (i-1)) + (rightENum VU.! i)| i <- [1..n]]
--   trace (show as) def
