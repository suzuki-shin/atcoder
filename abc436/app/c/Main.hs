...
今から M 回の操作を行います。 i 回目 (1≤i≤M) の操作は以下の通りです。
    マス (Ri​,Ci​) を左上とする 2×2 の領域を占めるブロックを、既に置かれている他のブロックと位置が重ならない場合、またその場合に限り、マス目の上に置く。より厳密には、マス集合 S={(Ri​,Ci​),(Ri​+1,Ci​),(Ri​,Ci​+1),(Ri​+1,Ci​+1)} に対し、既にマス目の上に置かれているブロックであって S に含まれるいずれかのマスを占めるものが存在するならば何も行わず、存在しないならば S に含まれる 4 マス全体を占めるブロックを置く。
...
@tags: [Set 近傍探索 座標圧縮不要]
@note: 2×2ブロックの重なり判定を左上座標の距離条件(差の絶対値≤1)に帰着。Setで配置済み座標を管理し、9マスのmemberチェックでO(M log M)
-}
{-# INLINE solve #-}
solve :: Solver
solve (n,m,rcs) = S.size res
  where
    initPos = S.empty -- 最初はひとつも置かれていない
    res = foldl' f initPos rcs -- おけた左上のマスのSet
    f :: S.Set (Int, Int) -> [Int] -> S.Set (Int, Int)
    f blockPoss [r,c] = if canPlace blockPoss (r,c) then S.insert (r,c) blockPoss else blockPoss
    f _ _ = error "invalid"
    canPlace :: S.Set (Int,Int) -> (Int,Int) -> Bool
    canPlace blockPoss (r,c) = not $ any (\(i,j) -> S.member (r+i,c+j) blockPoss) [(i,j)|i<-[-1,0,1], j<-[-1,0,1]]
