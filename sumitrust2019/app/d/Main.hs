...
{-# INLINE solve #-}
solve :: Solver
solve (_n, s) = length $ filter (`isSubseq` s) candidates
  where
    candidates = [B.pack [a, b, c] | a <- "0123456789", b <- "0123456789", c <- "0123456789"]
    isSubseq t x
      | B.null t = True
      | B.null x = False
      | B.head t == B.head x = isSubseq (B.tail t) (B.tail x)
      | otherwise = isSubseq t (B.tail x)
