...
{-# INLINE solve #-}
solve :: Solver
solve (n,xs) = map (yn . (`IS.member` prs)) xs
  where
    prs = IS.fromList $ primes $ maximum xs

{-# INLINE decode #-}
decode :: [[I]] -> Dom
decode = \case
  [n] : as -> (n, concat as)
  _ -> invalid $ "toDom: " ++ show @Int 83
