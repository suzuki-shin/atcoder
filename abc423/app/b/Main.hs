...
{-# INLINE solve #-}
solve :: Solver
solve (n, ls) = max ((n + 1) - (fromR + fromL)) 0
  where
    fromR = length (takeWhile (==0) ls) + 1
    fromL = length (takeWhile (==0) $ reverse ls) + 1

{-# INLINE decode #-}
decode :: [[I]] -> Dom
decode = \case
  [n] : ls : _ -> (n, ls)
  _ -> invalid $ "toDom: " ++ show @Int 82
