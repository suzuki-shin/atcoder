...
-- asを降順sortして偶数番目がAlice、奇数番目がBobのカードになる
solve :: Solver
solve (_, as) =
    let sorted = sortOn Down as
        alice = evenPositions sorted
        bob   = oddPositions sorted
    in sum alice - sum bob
...
positionsBy f xs = [x | (i, x) <- zip [0 ..] xs, f i]
