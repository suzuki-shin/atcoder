...
i 種類目のチーズは 1 [g] あたりのおいしさが Ai​ で、 Bi​ [g] あります。
...
-}
solve :: Solver
solve (n,w,abList) = fst res
  where
    sortedAbs = sortOn (Down . head) abList
    res = foldl' f (0, w) sortedAbs
    f :: (Int, Int) -> [Int] -> (Int, Int)
    f (v, restW) [a,b] = (v + a * b', restW - b')
      where
        b' = min restW b
