...
{-
@tags: [キュー Data.Sequence mapAccumL]
@note: Data.Sequenceでキュー操作(|>でenqueue、viewlでdequeue)。mapAccumLで状態(キュー)を持ちながらクエリを逐次処理し、出力はMaybeで表現してcatMaybesで抽出
-}
{-# INLINE solve #-}
solve :: Solver
solve (n, qs) = catMaybes mRes
  where
    ...
    f acc [1,x] = (acc Q.|> x, Nothing)
    f acc [2] = case Q.viewl acc of
      x Q.:< rest -> (rest, Just x)
      _ -> error "invalid"
    f _ _ = error "invalid"
