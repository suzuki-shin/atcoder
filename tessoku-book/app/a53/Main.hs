...
solve :: Solver
solve (_, qs) =
  catMaybes $ snd $ mapAccumL f H.empty qs
  where
    f :: H.Heap Int -> [Int] -> (H.Heap Int, Maybe Int)
    f heap (1 : price : _) = (H.insert price heap, Nothing)
    f heap (2 : _) = case H.uncons heap of
      Just (x, _) -> (heap, Just x)
      _ -> impossible "solve f 2"
    f heap (3 : _) = case H.uncons heap of
      Just (_, rest) -> (rest, Nothing)
      _ -> impossible "solve f 3"
