...
-- (全問回答済みユーザーのリスト, ユーザーごとの回答問題SetのMap（keyがユーザー、valueが回答問題のSet）)
type Acs = ([Int], IM.IntMap IS.IntSet)

{-# INLINE solve #-}
solve :: Solver
solve (n,m,k,xs) = reverse $ fst res
  where
    initAcs = IM.fromList $ map (\i -> (i, IS.empty)) [1..n]
    res = foldl' f ([], initAcs) xs
    f :: Acs -> [Int] -> Acs
    f (acUsers, acs) [a,b] =
      let mAcsOfA = IM.lookup a acs
      in case mAcsOfA of
        Just acsOfA -> if IS.size acsOfA >= m-1
          then (a:acUsers, IM.insert a (IS.insert b acsOfA) acs)
          else (acUsers, IM.insert a (IS.insert b acsOfA) acs)
        Nothing -> (acUsers, IM.insert a (IS.singleton b) acs)

{-# INLINE decode #-}
decode :: [[I]] -> Dom
decode = \case
  [n,m,k] : rest -> (n,m,k,rest)
  _ -> invalid $ "toDom: " ++ show @Int 100
