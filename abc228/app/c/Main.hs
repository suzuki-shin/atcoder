...
現在 3 日目までの試験が終わり、これから 4 日目の試験が行われようとしています。i(1≤i≤N) 番目の生徒は j(1≤j≤3) 日目の試験で Pi,j​ 点獲得しました。
...
-}
solve :: Solver
solve (n,k,pss) = map (yn . (>= threshold)) $ VU.toList pv
  where
    pv = VU.fromList $ map sum pss
    sortedPv = VU.modify (VAI.sortBy (comparing Down)) pv
    threshold = sortedPv VU.! (k-1) - 300
