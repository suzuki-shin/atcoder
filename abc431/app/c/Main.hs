...
現在、高橋くんは頭パーツを N 個と体パーツを M 個持っています。高橋くんが持っている i 番目 (1≤i≤N) の頭パーツの重さは Hi​ グラム、i 番目 (1≤i≤M) の体パーツの重さは Bi​ グラムです。
...
一番重い頭と一番重い体から貪欲法でやってみる
まずリストでやる。
とりあえずsampleは全部通った
計算量は？
sortOn'はわからない。O(N)とすると、foldl'がO(N)。だからO(N^2)か？それだとだめだね。
一番重いbodyより重いheadはいらない
あれ？通った
-}
{-
@tags: [貪欲法 ソート]
@note: 頭と体を降順ソートし、重い方からペアリングする貪欲法。sortOn'はO(N log N)なので全体もO(N log N)
-}
{-# INLINE solve #-}
solve :: Solver
solve x@(n,m,k,hs,bs) = trace (show x) trace (show res)
  yn $ robotsNum >= k
  where
    bsDsc = sortOn' Down bs
    hsDsc = dropWhile (head bsDsc <) $ sortOn' Down hs
    res@(robotsNum, _) = foldl' f (0, bsDsc) hsDsc
    f :: (Int, [Int]) -> Int -> (Int, [Int])
    f (robotsNum, b:restBs) h =
      if h <= b
        then (robotsNum + 1, restBs)
        else (robotsNum, b:restBs)
    f (robotsNum, []) _ = (robotsNum, [])
