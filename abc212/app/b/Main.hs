...
import Data.List.Extra (nubOn, nubOrd)
...
{-# INLINE solve #-}
solve :: Solver
solve xs = if isWeak then "Weak" else "Strong"
    where
        isWeak = sameAll || seqAll
        sameAll = 1 == length (nubOrd xs)
        seqAll = and $ zipWith isSeq xs $ tail xs
            where
                succOf n = (n+1) `mod` 10 -- if n == 9 then 0 else succ n
                isSeq a b = succOf a == b

{-# INLINE decode #-}
decode :: [[I]] -> Dom
decode = \ case
    xs:_ -> map digitToInt xs
    _   -> invalid $ "toDom: " ++ show @Int 66
