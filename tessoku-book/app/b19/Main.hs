...
solve (n, maxW, wvs) =
  let maxV = n * 1000
      -- 価値を添字（インデックス）にして、最小の重さを値として持つDP
      dp :: VU.Vector Int
      dp = foldl' step initVec wvs
      initVec :: VU.Vector Int
      initVec = VU.replicate (maxV + 1) maxBound
      step :: VU.Vector Int -> (Int, Int) -> VU.Vector Int
      step prevVec (w, v) = VU.generate (maxV + 1) f
        where
          -- 目標の価値v'を達成する最小の重さを計算する
          f 0 = 0
          f v' =
...
decode :: [[I]] -> Dom
decode = \case
  [n, maxW] : as -> (n, maxW, wvs)
    where
      wvs = map (\[w, v] -> (w, v)) as
  _ -> invalid $ "toDom: " ++ show @Int 121
