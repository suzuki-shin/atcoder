...
type Dom = (Int, Int, String, String, Int, [String])
...
solve (n,m,s,t,q,ws) = map check ws
  where
    ...
    aokiS = S.fromList t
    inTaka :: String -> Bool
    inTaka = all (`S.member` takaS)
    inAoki :: String -> Bool
    inAoki = all (`S.member` aokiS)
    check :: String -> String
    check word = case (inTaka word, inAoki word) of
      (True, True) -> "Unknown"
...
decode :: [[I]] -> Dom
decode = \case
  nm : s : t : q : rest -> (read n, read m, s, t, read q, rest)
    where
      [n,m] = words nm
  _ -> invalid $ "toDom: " ++ show @Int 87
