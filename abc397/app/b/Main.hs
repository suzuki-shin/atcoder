...
{-
  @tags: [貪欲法]
  @note: 状態(次に期待する文字)を持ちながら1文字ずつ処理する貪欲法。元文字列のインデックスではなく結果文字
  列の位置を状態で追跡するのがポイント。末尾の未ペア処理も忘れずに
  -}
{-# INLINE solve #-}
solve :: Solver
solve s = addNum + if expect == 'o' then 1 else 0
  where
    (expect, addNum) = foldl' f ('i', 0) s
    f :: (Char, Int) -> Char -> (Char, Int)
    f ('i', num) c = if 'i' == c then ('o', num) else ('i', num + 1)
    f ('o', num) c = if 'o' == c then ('i', num) else ('o', num + 1)
    f _ _ = error "invalid"
...
instance (Ord s, Semiring s) => Semiring (MinPlusWithRoute s p) where
  ...
  {-# INLINE zero #-}
  zero :: (Ord s, Semiring s) => MinPlusWithRoute s p
