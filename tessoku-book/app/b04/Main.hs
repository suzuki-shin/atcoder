...
-}
solve :: Solver
solve ns = sum $ zipWith (\ i n -> n * 2 ^ i) [(0::Int) .. 7] $ reverse ns
-- solve ns = sum $ map (\(i, n) -> n * 2 ^ i) $ zip [(0 :: Int) .. 7] $ reverse $ ns

{-
>>> sum $ map (\(i, n) -> n * 2^i) $ zip [0..7] $ reverse $ decode ["1101"]
13

>>> zip [0..7] $ reverse $ decode ["1101"]
[(0,1),(1,0),(2,1),(3,1)]
