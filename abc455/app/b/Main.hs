...
solve :: Solver
solve (h,w,ss) =
  countIf isPointSymmetric rectangles
  where
    rectangles = [(h1,h2,w1,w2)| h1 <- [1..h], h2 <- [h1..h], w1 <- [1..w], w2 <- [w1..w]]
    isPointSymmetric (h1,h2,w1,w2) =
      all sameAsMirror cells
      where
        cells = [(i,j)| i <- [h1..h2], j <- [w1..w2]]
        sameAsMirror (i,j) = cell i j == cell (h1+h2-i) (w1+w2-j)
        cell i j = ss !! (i-1) !! (j-1)
