...
{-
@tags: [グリッド回転 全探索]
@note: 90度右回転(transpose . reverse)を0〜3回適用した4パターンを全探索。各回転で色が違うマス数+回転回数の最小値が答え。最小化するのは「差分数だけ」でなく「回転回数+差分数」であることに注意
-}
{-# INLINE solve #-}
solve :: Solver
solve (n, s, t) = minimum [r + d | (r, d) <- zip [0 ..] ds]
  where
    -- 色が違うマスの数
    difNum :: [[Char]] -> [[Char]] -> Int
    difNum s' t' = count True $ zipWith (/=) (concat s') (concat t')
    -- 90度右回転する
    rotateR90 :: [[Char]] -> [[Char]]
    rotateR90 = transpose . reverse

    ds = map (difNum t) $ take 4 $ iterate rotateR90 s
