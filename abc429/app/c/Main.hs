...
1≤i<j<k≤N をみたす整数の組 (i,j,k) であって、次の条件をみたすものの個数を求めてください。
    ...
    Ai​,Aj​,Ak​ の中にちょうど 2 種類の値が含まれる。すなわち、Ai​,Aj​,Ak​ の中のいずれか 2 つは等しく、残りの 1 つは異なる。
...
-}
{-
@tags: [数え上げ 組み合わせ 主客転倒]
@note: 各値vの出現回数c[v]について C(c[v],2)*(N-c[v]) を合計する。「値vが同じペアを担当する組の数」を値ごとに数える主客転倒。積の法則(独立な選択は掛け算)がポイント
-}
{-# INLINE solve #-}
solve :: Solver
solve (n, as) = res
  where
    freq :: IM.IntMap Int
    freq = IM.fromListWith (+) $ map (,1) as
    res = sum $ IM.map (\cnt -> comb2 cnt * (n - cnt)) freq
