...
{-# INLINE solve #-}
solve :: Solver
solve (h,w,k,grid) = maximum res
  where
    res = [blackCount mask | mask <- [0 .. bit h - 1], popCount mask <= k]
    blackCount :: Int -> Int
    blackCount mask = h * w - leftover
      where
        r = popCount mask -- 塗る行数
        remaining = k - r -- 列に使える残り操作回数
        unselectedRows :: [Int] -- maskで選ばれていない行のリスト
        unselectedRows = [i | i <- [0..h-1], not (testBit mask i)]
        countWhite :: [Int] -> Int -> Int -- 指定された行たちにおける、列jの白マス数
        countWhite rows j = length [() | i <- rows, (grid !! i) !! j == '.']
        whiteCounts :: [Int] -> [Int] -- 列ごとの白マス数を降順ソートしたリスト
        whiteCounts rows = sortBy (flip compare) [countWhite rows j | j <- [0..w-1]]
        sorted = whiteCounts unselectedRows -- 各列の白マス数（降順）
        leftover = sum (drop remaining sorted) -- 濡れなかった列の白マス数
