...
{-# INLINE solve #-}
solve :: Solver
solve (_, qs) = catMaybes $ snd $ mapAccumL f Q.empty qs
  where
    f :: Q.Seq String -> (Int, String) -> (Q.Seq String, Maybe String)
    f queue (1, name) = (queue Q.|> name, Nothing)
    f queue (2, _) = case Q.viewl queue of
      x Q.:< _ -> (queue, Just x)
      _ -> impossible "solve f 2"
    f queue (3, _) = case Q.viewl queue of
      _ Q.:< rest -> (rest, Nothing)
