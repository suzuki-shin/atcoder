...
{-# INLINE solve #-}
solve :: Solver
solve (h,w,grid,q,rss) =
  let csum = csum2 (h,w) grid
  in [csum +!! ((a-1,b-1),(c-1,d-1)) |[a,b,c,d] <- rss]
-- trace (show x) def

{-# INLINE decode #-}
decode :: [[I]] -> Dom
decode = \case
  [h,w] : rest -> (h, w, grid, q, rss)
    where
      grid = take h rest
      q = head (rest !! max 0 h)
      rss = drop (h+1) rest
  _ -> invalid $ "toDom: " ++ show @Int 69
...
-- | 1 次元の累積和配列を作成する。
-- >>> csum1 4 [1..10] :: UArray Int Int
-- array (0,4) [(0,0),(1,1),(2,3),(3,6),(4,10)]
csum1 :: (IArray UArray e, Num e) => Int -> [e] -> UArray Int e
csum1 n = listArray (0, n) . scanl' (+) 0
{-# INLINE csum1 #-}

-- | 1 次元の累積和配列を元に区間和を求める。
-- >>> let csum = csum1 4 [1..10] :: UArray Int Int
-- >>> csum +! (1, 2)
-- 5
