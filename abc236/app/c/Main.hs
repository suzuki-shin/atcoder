...
type Dom   = (Int, Int, [String], [String])
type Codom = [String]
...
decode :: [[I]] -> Dom
decode = \case
  [n',m'] : ss : ts : _ -> (read n', read m', ss, ts)
...
AtCoder 鉄道のとある路線には N 個の駅が存在し、始点から終点に向かって i(1≤i≤N) 番目の駅の名前は Si​ です。

普通列車は全ての駅に止まりますが、急行列車は全ての駅に止まるとは限りません。具体的には、急行列車は M(M≤N) 個の駅にのみ止まり、j(1≤j≤M) 番目に止まる駅の名前は Tj​ です。
ただし、T1​=S1​ かつ TM​=SN​、すなわち急行列車は始点と終点の両方に止まることが保証されます。
...
制約
    ...
    Si​=Sj​(i=j)
    T1​=S1​ かつ TM​=SN​
    (T1​,…,TM​) は (S1​,…,SN​) から 0 個以上の文字列を選んで取り除き、残った文字列を元の順序で並べることで得られる
