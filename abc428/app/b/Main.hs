...
solve (n,k,s) = (x, ss)
  where
    ssWithCount = M.fromListWith (+) $ map (,1) $ substringsK k s
    x = maximum ssWithCount
    ss = M.keys $ M.filter (== x) ssWithCount
...
decode = \case
  nk : s : _ -> (n, k, s)
    where
      [n,k] = map (read @Int) $ words nk
  _ -> invalid $ "toDom: " ++ show @Int 80

{-# INLINE encode #-}
encode :: Codom -> [[O]]
encode (a, bs) = [show a, unwords bs]
