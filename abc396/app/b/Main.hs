...
{-
  @tags: [スタック シミュレーション]
  @note: カードの山をリストで表現。先頭への追加/削除でスタック操作。初期状態は0が100枚。
-}
solve :: Solver
solve (_, qs) = catMaybes mRes
  where
    (_, mRes) = mapAccumL f (replicate 100 0) qs
    f :: [Int] -> [Int] -> ([Int], Maybe Int)
    f stack [1, x] = (x : stack, Nothing)
    f (y : rest) [2] = (rest, Just y)
    f stack query = trace (show stack) trace (show query) error "invalid"
